...
This document provides recipes to deploy a vLLM inference server on GKE. The core of this project is an architecture that loads models from a **GCS bucket** and caches them on a **Persistent Volume**.

## Architecture: GCS-to-Persistent-Disk Caching

The key architectural pattern in this project is the use of an `initContainer` to download a model from a GCS bucket to a GCE Persistent Disk. This provides two main benefits:

*   **Fast Initial Startup:** Downloading from GCS is significantly faster than from a public model repository.
*   **Near-Instant Subsequent Startups:** When a pod restarts, the model is already on the persistent disk, eliminating the need for re-downloading.

This GCS-to-Persistent-Disk caching strategy is the recommended approach for serving large models on GKE, providing a balance of performance, reliability, and ease of use.
...
## Design History and Troubleshooting
